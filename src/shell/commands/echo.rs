/// Handles the echo command with all its common flags
pub fn handle_echo_command(cmd: &str) -> String {
    // Skip 'echo' part of the command
    let mut args = cmd.trim_start_matches("echo").trim();
    ...
    if print_version {
        return "echo (GNU coreutils) 8.32\n\
            ...
            '\\' if (enable_escapes && !in_single_quotes) && i + 1 < chars.len() => {
                ...
                match chars[i] {
                    ...
                    'c' => {
                        ...
                        return processed_output; // Return without newline
                    },
